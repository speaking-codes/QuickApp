...
select	COUNT_BIG(*)
		...
		--p.reddito, cl.cod_fascia_reddito, pr.sigla_provincia, pr.cod_regione, 'A01'
from	PERSONA p with (nolock) join CLIENTE cl with (nolock) 
on		p.codice_cliente = p.codice_cliente and p.sesso is not null left join PROVINCIA pr with (nolock)
...
on		mpc.cod_agenzia = dap.cod_agenzia and mpc.cod_ramo = dap.cod_ramo and mpc.numero_polizza = dap.numero_polizza 
and		(DATEPART(YEAR, dap.data_emissione_contratto) = '2024') and DATEPART(MONTH, dap.data_emissione_contratto) = '3' and dap.cod_stato_contratto = '01' join POLIZZA_AUTO pa
on		dap.cod_agenzia = pa.cod_agenzia and dap.cod_ramo = pa.cod_ramo and mpc.numero_polizza = pa.numero_polizza and pa.cod_settore_tariffario = '1' 
...
select	'insert	into Temp(Gender, BirthDate, MaritalStatusCode, FamilyTypeCode, ChildrenNumbers, IncomeTypeCode, ProfessioneTypeCode, ProfessioneTypeCode_1, ProfessioneTypeCode_2,
		...
		case when RegionCode is null then 'null, ' else '''' + RegionCode + ''', ' end + 'A01'
